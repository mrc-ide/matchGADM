#' fucntion that approximately matches shape file areas through a grid of points
#'
#'@param shp1_old Shape files of old gadm system
#'@param shp1_new Shape files of new gadm system
#'@param increment Width of the grid squares
#'
#'@return proportion of old shapefiles in new shapefiles (approximate)

grid_match = function(shp1_old,
                      shp1_new,
                      increment= 0.15){

  ### check increment is not too big ### ------------------------------------------------------ -------------------
  if(increment>0.3){message("Are you sure you want an increment this large, the results may be very inaccurate.")}

  ### REMOVE ACCENTS FOR MATCHING and COLLECT COUNTRIES ### -------------------------------------------------------
  out = tidy_shp(shp1_new, shp1_old)

  ### MAKE DATAFRAMES ### -----------------------------------------------------------------------------------------
  out = make_shp_dataframes(out$shp1_new, out$shp1_old)
  dat_new = out$dat_new
  dat_old = out$dat_old

  ### get grid extent ### -----------------------------------------------------------------------------------------
  grid_min_x = grid_min_y = grid_max_x = grid_max_y = NA
  for( p in 1:nrow(dat_old)){
    grid_min_x = min( rbind( out$shp1_old@polygons[[p]]@Polygons[[1]]@coords[,1] , grid_min_x), na.rm = TRUE)
    grid_max_x = max( rbind( out$shp1_old@polygons[[p]]@Polygons[[1]]@coords[,1] , grid_max_x), na.rm = TRUE)
    grid_min_y = min( rbind( out$shp1_old@polygons[[p]]@Polygons[[1]]@coords[,2] , grid_min_y), na.rm = TRUE)
    grid_max_y = max( rbind( out$shp1_old@polygons[[p]]@Polygons[[1]]@coords[,2] , grid_max_y), na.rm = TRUE)
  }

  ### make grid ### -----------------------------------------------------------------------------------------------
  x_coords = seq(grid_min_x, grid_max_x+0.1, by = increment)
  y_coords = seq(grid_min_y, grid_max_y+0.1, by = increment)

  grid_coords = expand.grid(x_coords, y_coords)
  names(grid_coords) = c("x", "y")

  ### check points are in polygons ### ----------------------------------------------------------------------------
  check_points = function(p){
    tmp = sp::point.in.polygon(grid_coords$x,
                               grid_coords$y,
                               shp1_old@polygons[[p]]@Polygons[[1]]@coords[,1],
                               shp1_old@polygons[[p]]@Polygons[[1]]@coords[,2])
  }

  ## see if they are in each old polygon
  grid_in_poly = sapply(1:nrow(dat_old), FUN = check_points)

  ### distribute in new polygons ### ------------------------------------------------------------------------------
  distribute_points = function(q){

    ind = which(grid_in_poly[,p]>0)

    tmp = sp::point.in.polygon(grid_coords[ind,"x"],
                               grid_coords[ind,"y"],
                               shp1_new@polygons[[q]]@Polygons[[1]]@coords[,1],
                               shp1_new@polygons[[q]]@Polygons[[1]]@coords[,2])

    sum(tmp, na.rm = TRUE)/ nrow(grid_coords[which(grid_in_poly[,p]>0),])
  }

  distribute_points_all = function(p){
    sapply(1:nrow(dat_new),distribute_points)
  }

  proportions = sapply(1:nrow(dat_old), distribute_points_all)

  ### sort out proportions ### -----------------------------------------------------------------------------------------
  proportions_df = as.data.frame(proportions)
  rownames(proportions_df) = dat_new$ID_1
  colnames(proportions_df) = dat_old$ID_1

  propn_out = cbind( reshape2::melt(proportions_df, id.vars=NULL), "new" = rep(dat_new$ID_1, nrow(dat_old)))

  propn_out = dplyr::filter(propn_out, value>0)

  propn_out = propn_out[,c(1,3,2)]

  return(propn_out)

}
